...
public class VectorTest {
	...
	public void spTestData() {
		...
		ch.epfl.dias.ops.vector.Select sel = new ch.epfl.dias.ops.vector.Select(scan, BinaryOp.EQ, 3, 4);
		//sel.open();
		/*
		System.out.println("here");
		while(res) {
			DBColumn[] cols = sel.next();
		    if(cols[0].eof ) {
		    	res = false;
	...
	public void joinTest1() {
		...
		i=0;
		do {
			DBColumn[] res = selLineitem.next();
			if(res[0].eof)break;
					i+=res[0].fields.size();
		}while(true);
		System.out.println("count 2 is"+i);
